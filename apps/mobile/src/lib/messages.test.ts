 ...
 * it the way the `generate` script does for Metro. When it does not, every
 * component that reads a message fails to load with "Failed to resolve import
...
describe("message catalog", () => {
  it("resolves a message in a test run", () => {
    expect(m.app_title()).toBe("Hype Stack");
  });
});
