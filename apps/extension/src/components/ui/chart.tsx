...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>{labelFormatter(value as string, payload as never[])}</div>
      );
    }
    ...
    <div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item: Record<string, unknown>) => {
        ...
        return (
          ...
          >
            {itemConfig?.icon && !hideIcon ? <itemConfig.icon /> : <ChartLegendSwatch color={item.color as string} />}
            {itemConfig?.label}
          </div>
        );
      })}
