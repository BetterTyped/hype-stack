...
import { defineQueue, enqueue, startQueues, stopQueues, type Task } from "./queue";

// Needs the test Postgres and Valkey (`pnpm test:setup`). The `.integration.test.ts` name is what
// keeps it out of runs that have no services, like a fresh checkout or the registry e2e lanes.
