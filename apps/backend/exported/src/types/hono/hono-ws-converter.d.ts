...
type NestListeners<Listeners extends Record<string, any>, S extends SocketInstance> = Prettify<UnionToIntersection<{
    [Topic in keyof Listeners & string]: BuildNestedObject<SplitPath<Topic>, {
        $listener: Listener<Listeners[Topic], Topic, S>;
    }>;
}[keyof Listeners & string]>>;
type NestEmitters<Emitters extends Record<string, any>, S extends SocketInstance> = Prettify<UnionToIntersection<{
    [Topic in keyof Emitters & string]: BuildNestedObject<SplitPath<Topic>, {
        $emitter: Emitter<Emitters[Topic], Topic, S>;
    }>;
}[keyof Emitters & string]>>;
type DeepMerge<A, B> = {
    [K in keyof A | keyof B]: K extends keyof A & keyof B ? A[K] extends Record<string, any> ? B[K] extends Record<string, any> ? DeepMerge<A[K], B[K]> : A[K] : A[K] : K extends keyof A ? A[K] : K extends keyof B ? B[K] : never;
};
type EventsToSocketSdk<Events, S extends SocketInstance> = Events extends {
    listeners: infer L extends Record<string, any>;
    emitters: infer E extends Record<string, any>;
} ? Prettify<DeepMerge<NestListeners<L, S>, NestEmitters<E, S>>> : never;
...
type SocketRoutesToEvents<T extends readonly (readonly [string, SocketRouter<any>])[]> = {
    listeners: UnionToIntersection<{
        [I in keyof T]: T[I] extends readonly [infer P extends string, infer R] ? PrefixKeys<ExtractBackendEmitters<R>, P> : never;
    }[number]>;
    emitters: UnionToIntersection<{
        [I in keyof T]: T[I] extends readonly [infer P extends string, infer R] ? PrefixKeys<ExtractBackendListeners<R>, P> : never;
    }[number]>;
};
/**
 ...
 * ```
 */
export type SocketRoutesToSdk<T extends SocketRouter<any, any> | readonly (readonly [string, SocketRouter<any>])[], S extends SocketInstance> = T extends SocketRouter<any, any> ? EventsToSocketSdk<SocketRoutesToEvents<ExtractRoutes<T>>, S> : T extends readonly (readonly [string, SocketRouter<any>])[] ? EventsToSocketSdk<SocketRoutesToEvents<T>, S> : never;
 ...
 */
export type ServerEmitterMap<R extends SocketRouter<any, any>> = ExtractRoutes<R> extends readonly [] ? Record<string, never> : CollectEmitters<ExtractRoutes<R>>;
export {};
//# sourceMappingURL=hono-ws-converter.d.ts.map