...
function getThemeTransitionClipPaths(opts: {
  variant: TransitionVariant;
  cx: number;
  cy: number;
  maxRadius: number;
  ...
  const { variant, cx, cy, maxRadius, viewportWidth, viewportHeight } = opts;
  switch (variant) {
    case "circle":
...
export const AnimatedThemeToggler = ({
  ...
  return (
    <button
      ...
      onClick={toggleTheme}
      className={cn(
        "inline-flex items-center justify-center cursor-pointer h-8 w-8 rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors",
        className,
      )}
      {...props}
    >
